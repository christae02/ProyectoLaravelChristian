...
@section('content')
  <div class="flex-col mb-37 mt-10 justify-items-center text-center w-full">
    ...
    <div class="w-[40%] p-10 bg-gray-200 rounded-4xl shadow-lg">
        <h1 class="text-4xl font-bold text-blue-500 mb-5">EDITAR INFORMACIÓN ANTIBIOTICO</h1>
        <form 
            class="bg-gray" 
            method="POST" 
            action="{{ route('antibioticos.update',$medicamento->id) }}"
        >
            @method('POST')
            @csrf


            <x-forminput title="Nombre" id="nombre" value="{{ old('nombre',$medicamento->nombre) }}" mb="0" width="120"/>
            <x-errormessage attribute="nombre"/>

            <x-forminput title="Miligramos" id="mg" value="{{ old('mg',$medicamento->mg) }}" mb="0"/>
            <x-errormessage attribute="mg"/>

            <x-forminput title="Presentación" id="presentacion" value="{{ old('presentacion',$medicamento->presentacion) }}" mb="0"/>
            <x-errormessage attribute="presentacion"/>

            <x-forminput title="Imagen" id="imagen" type="file" mb="0" value="{{ old('imagen') }}"/>
